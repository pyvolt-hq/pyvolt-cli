...
| `pyvolt servers select NAME` | Scope later commands to one server (`--clear` to undo) |
...
`DOMAIN` is the app's domain, or any unambiguous fragment of it. Every
app-addressed command also takes `-s SERVER` to scope one call, or use
`pyvolt servers select` for a sticky context.
